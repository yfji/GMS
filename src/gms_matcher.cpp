...
vector<DMatch> matchImage(Mat& image1, Mat& image2, vector<KeyPoint>& kp1, vector<KeyPoint>& kp2, const int top_n) {
	Mat d1, d2;
	...
	if (image1.rows * image1.cols > 480 * 640) {
	...
	gms_matcher gms(kp1, image1.size(), kp2, image2.size(), matches_all);
	...
	std::sort(matches_gms.begin(), matches_gms.end(), [](DMatch& d1, DMatch& d2) {
		return (d1.distance < d2.distance);
	});
	int N=MIN(matches_gms.size(), top_n);
	std::vector<DMatch> selected_matches(N);
	for (int i = 0; i < N; ++i) {
		selected_matches[i] = matches_gms[i];
	}
